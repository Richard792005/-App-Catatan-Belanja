...
const gro = groceryItems.map((item) => {
  return {...item, halo: 'Nama'}
})
...
export default function App() {
  ...
  function handleDeleteItem(id) {
    setItems((items) => items.filter((item, i) => {
      return item.id !== id
    }))
  }
  ...
  function handleClearItem() {
    alert('List barang dihapus !')
    setItems([])
    setItems((items) => items.filter((item) => false))
  ...
  return (
    <>
      ...
      <GroceryList items={items} onDeleteItems={handleDeleteItem} onCheckedItems={handleCheckedItem} onClearItems={handleClearItem}/>
      <Footer />
    </>
  )
}
...
function Form({ onAddItems }) {
  ...
  const quantityNum = [...Array(10)].map((_, i) => (
    <option value={i + 1} className="">{i + 1}</option>
  ))
  ...
  function handleSubmit(e) {
    ...
    //kalau kosong input , stop jangan jalankan
    if (!name) {
      return;
    }
    ...
    //panggil fungsi nya yang ad di function app
    onAddItems(newItem)
...
function GroceryList({ items, onDeleteItems, onCheckedItems, onClearItems }) {
    ...
    <>
        ...
        <div className="List-item py-2 w-lg h-[29.78rem] mx-auto text-center">
          <ul>
            {sortedItems.map((item, i) => (
              <Item onDeleteItems={onDeleteItems} onCheckedItems={onCheckedItems} item={item} key={item.id} />
            ))}
          </ul>
        </div>
      </section>
      <section id="" className="flex py-3 justify-center bg-lime-700 text-dark lg:text-base">
        <div className="flex flex-wrap gap-5 action justify-center">
          ...
          <button onClick={onClearItems} type="button" className='py-2 px-4 rounded-full bg-amber-100 max-w-[32%] hp:max-w-[39%]'>Bersihkan Daftar</button>
        </div>
      </section>
    </>
  )
...
function Item({ item, onDeleteItems, onCheckedItems }) {
  return (
    <li className="mb-2 flex justify-center items-center">
      <label onClick={() => {onCheckedItems(item.id)}} htmlFor="toggle" className="">
      ...
      <span onClick={() => onDeleteItems(item.id)} className="">
  ...
  return (
    <>
      <footer className="static w-full stats bg-yellow-900 flex justify-center items-center py-6 lg:py-10 ">
        <span className="text-white text-lg sm:text-lg font-semibold  sm:max-w-full ">Ada 10 Barang di daftar belanjaan <span className="hp:block sm:inline">5 barang sudah dibeli (50%)</span></span>
      </footer>
    </>
  )
}