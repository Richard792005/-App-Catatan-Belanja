...
export default function App() {
  ...
  function handleCheckedItem(id) {
    setItems((items) => items.map((item) => {
      return { ...item, checked: !item.checked, name: 'klik-semua' }
    }))
  }
  ...
  return (
    <>
      ...
      <GroceryList item={items} onCheckedItem={handleCheckedItem} onDeleteItem={handleDeleteItem} onResetItem={handleResetItem} />
      <Footer item={items} />
    </>
  )
}
...
function Form({ onAddItems }) {
  // membuat Quantity generate
  const generateQuantity = [...Array(10)].map((_, i) => (
    ...
    <section id="input-barang" className="bg-amber-500  py-4 lg:py-7 items-center hp:pl-4  sm:pl-11 lg:pl-0 font-['Poppins']">
      <form onSubmit={handleSubmit} action="" className="hp:grid grid-cols-2 lg:flex flex-wrap justify-center items-center">
        ...
        <div className=" bg-amber-100 text-stone-600 h-10 rounded-2xl max-w-20 py-5 lg:mr-2 col-start-1 col-end-2 row-start-2 row-end-3 flex justify-center items-center">
          <select value={quantity} onChange={(e) => setQuantity(e.target.value)} name="number" id="number-select" className='lg:px-2 hp:px-4 justify-center flex focus:outline-none active:outline-none truncate'>
            {generateQuantity}
          </select>
        </div>
...
function GroceryList({ item, onCheckedItem, onDeleteItem, onResetItem }) {
  ...
  const handleChange = (e) => {
    ...
    setForm(prev => ({ ...prev, input: null, [e.target.name]: e.target.value }))
        ...
        <div className="List-item py-2 w-lg h-[29.78rem] mx-auto text-center">
          <ul>
            {sortedItems.map(item => (
              <Item item={item} onCheckedItem={onCheckedItem} onDeleteItem={onDeleteItem} />
            ))}
      ...
      <section id="" className="flex py-3 justify-center bg-lime-700 text-dark lg:text-base">
        <div className="flex flex-wrap gap-5 action justify-center">
          <div className="sm:max-w-2/3  bg-amber-100 rounded-full py-2 px-5">
            <select name='form' value={form.input} onChange={handleChange} className='focus:outline-none'>
              <option value="input">Urutkan berdasarkan urutan input</option>
              <option value="name">Urutkan berdasarkan nama barang</option>
              <option value="checked">Urutkan berdasarkan ceklis</option>
            </select>
          </div>
          <button onClick={onResetItem} type="button" className='py-2 px-4 rounded-full bg-amber-100 max-w-[32%] hp:max-w-[39%]'>Bersihkan Daftar</button>
        </div>
      </section>
    </>
  )
...
function Item({ item, onCheckedItem, onDeleteItem }) {
  return (
    <li key={item.id} className="mb-2 flex justify-center items-center">
      <label onClick={() => onCheckedItem(item.id)} htmlFor="toggle" className="">
        <input checked={item.checked} type="checkbox" name="" id="toggle" className='inline-block mr-4 scale-150' />
      ...
      <span onClick={() => onDeleteItem(item.id)} className="">
